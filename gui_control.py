...
class VisualGamepadApp:
    ...
    def _build_ui(self):
        ...
        cfg_btn = tk.Button(
            btn_bar,
            text="⚙️ Abrir Mapeo y Calibración (Qt GUI)",
            font=("Segoe UI", 10, "bold"),
            bg="#bd93f9",
            fg="#ffffff",
        ...
        cfg_btn.pack(side=tk.LEFT, fill=tk.X, expand=True)
    ...
    def _open_qt_config(self):
        """Abre la ventana modal Qt de GamepadMapper en un hilo separado o directo."""
        threading.Thread(target=self._run_qt_dialog, daemon=True).start()

    def _run_qt_dialog(self):
        try:
            self.pad.show_config_dialog()
        except Exception as e:
            print(f"Error al abrir diálogo Qt: {e}")
